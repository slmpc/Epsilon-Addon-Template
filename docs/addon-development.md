...
NeoForge 端建议在 `@Mod` 启动类中直接向 `NeoForge.EVENT_BUS` 注册监听：
...
@Mod(value = "your_mod_id", dist = Dist.CLIENT)
public class ExampleNeoMod {

    public ExampleNeoMod(ModContainer container) {
